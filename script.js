...
let weather = {
    ...
    displayWeather: function (data) {
        ...
        document.querySelector(".city").innerHTML = "<span>Weather in</span> " + name + ", " + country;
        document.querySelector(".icon").src = "https://openweathermap.org/img/wn/" + icon + "@2x.png";
        document.querySelector(".description").innerText = description;
        document.querySelector(".temp").innerText = temp + " °C";
        document.querySelector(".feel").innerHTML = "<span>Feels like:</span> " + feels_like + " °C";
        document.querySelector(".max").innerHTML = "<span>Max:</span> " + temp_max + " °C";
        document.querySelector(".min").innerHTML = "<span>Min:</span> " + temp_min + " °C";
        document.querySelector(".humidity").innerHTML = "<span>Humidity:</span> " + humidity + "%";
        document.querySelector(".wind").innerHTML = "<span>Wind speed:</span> " + speed + " k/h";
    ...
    background_src: function (source) {
        if (window.innerWidth > 1080) {
            document.body.style.backgroundImage = "url('https://source.unsplash.com/1920x1080/?" + source + "')";
        } else {
            document.body.style.backgroundImage = "url('https://source.unsplash.com/1080x1920/?" + source + "')";
    ...
    reverseGeocode: function (latitude, longitude) {
        ...
        request.onload = function () {
            ...
            if (request.status === 200) {
                ...
                } else if (typeof data.results[0].geometry.lat != 'undefined' && typeof data.results[0].geometry.lng != 'undefined') {
                    weather.fetchWeatherC(data.results[0].geometry.lat, data.results[0].geometry.lng);
                } else {
                    alert("Do you even live here \" " + data.results[0].formatted + " \"?")
                }

                document.querySelector(".location").innerText = "Current location: " + data.results[0].formatted + " (" + data.results[0].geometry.lat + ", " + data.results[0].geometry.lng + ")";
